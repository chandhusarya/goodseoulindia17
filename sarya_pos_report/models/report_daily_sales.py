...
class ReportDailySale(models.AbstractModel):

    _name = 'report.sarya_pos_report.report_dailysales'
    _description = 'Point of Daily Sale'
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
        ...
        for order in orders:
            ...
            # label_product = self.env.ref("sh_pos_all_in_one_retail.product_product_demo_product_label_line")

            for line in order.lines:
                # if line.product_id.id == label_product.id:
                #     continue
                if line.qty >= 0:
        ...
        # order_types = self._get_order_type(orders.grouped('sh_order_type_id'))
        ...
        if payment_ids:
            ...
            payments = []
        payment_summary = {}
        payment_names = [d['name'] for d in payments if 'name' in d]
        for pay in payment_names:
            payment_summary[pay] = sum(d.get('total', 0) for d in payments if d.get('name', '') == pay)
        if payment_summary:
            payment_summary['Total'] = round(sum(d.get('total', 0) for d in payments), 2)
    ...
    # def _get_order_type(self, orders):
    #     order_types = {}
    #     count_total = 0
    #     sales_total = 0
    #     for order in orders:
    #         order_types.setdefault(order.name or 'Undefined', {})
    #         order_types[order.name or 'Undefined']['count'] = len(orders[order])
    #         order_types[order.name or 'Undefined']['sales'] = round(sum(orders[order].mapped('amount_total')), 2)
    #         count_total += len(orders[order])
    #         sales_total += round(sum(orders[order].mapped('amount_total')), 2)
    #     if order_types:
    #         order_types.setdefault('Total', {})
    #         order_types['Total']['count'] = count_total
    #         order_types['Total']['sales'] = sales_total
    #     return order_types
    ...
    def _get_products_and_taxes_dict(self, line, products, taxes, currency):
        ...
        if line.combo_line_ids:
            for combo in line.combo_line_ids:
                new_price += combo.price_unit
        elif line.combo_parent_id:
            new_price = line.combo_parent_id.price_unit
        else:
            new_price = line.price_unit
        key2 = (line.product_id, new_price, line.discount)
        ...
        if line.tax_ids_after_fiscal_position:
            ...
            for tax in line_taxes['taxes']:
                taxes.setdefault(tax['id'], {'name': tax['name'], 'tax_amount':0.0, 'base_amount':0.0, 'order_line_id': False})
                taxes[tax['id']]['tax_amount'] += tax['amount']
                taxes[tax['id']]['base_amount'] += tax['base']
                taxes[tax['id']]['order_line_id'] += line.id
        else:
            taxes.setdefault(0, {'name': _('No Taxes'), 'tax_amount':0.0, 'base_amount':0.0, 'order_line_id': False})
            taxes[0]['base_amount'] += line.price_subtotal_incl
            taxes[0]['order_line_id'] += line.id
    ...
    def _get_taxes_info(self, taxes):
        ...
        for tax in taxes.values():
            ...
            if tax['order_line_id'] not in order_line_id:
                total_base_amount += tax['base_amount']
                order_line_id.append(tax['order_line_id'])
