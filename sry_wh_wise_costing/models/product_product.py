...
class ProductProduct(models.Model):
    ...
    def _run_fifo(self, quantity, company):
        ...
        warehouse_id = self.env.context.get('warehouse_id_valuation', False)

        if not warehouse_id:
            raise UserError("Cannot determine warehouse for FIFO valuation. Please ensure warehouse_id_valuation is set in the context.")
        ...
        for candidate in candidates:
            ...
            print("new_remaining_value =========>> ", new_remaining_value)
        ...
        if self.cost_method == 'fifo':
            ...
            elif candidates and not float_is_zero(qty_to_take_on_candidates,
    ...
    def _run_fifo_vacuum(self, company=None):
        ...
        for svl_to_vacuum in svls_to_vacuum:
            ...
            candidates = all_candidates.filtered(
                lambda r: r.warehouse_id.id == svl_to_vacuum.warehouse_id.id
                          and (
                                  r.create_date > svl_to_vacuum.create_date
                                  or (
                                          r.create_date == svl_to_vacuum.create_date
    ...
    def _prepare_intenal_wh_svl_vals(self, quantity, company, from_warehouse_id, to_warehouse_id):
        ...
        vals = {
            ...
            'quantity': quantity,
            'force_warehouse': to_warehouse_id
        }
        fifo_vals = self.with_context(warehouse_id_valuation=from_warehouse_id)._run_fifo(abs(quantity), company)

        print("fifo_vals =========>> ", fifo_vals)

        #Redo value for internal transfer
        #vals['remaining_qty'] = fifo_vals.get('remaining_qty')

        fifo_vals['value'] = abs(fifo_vals.get('value'))

        if self.product_tmpl_id.cost_method == 'fifo':
            vals.update(fifo_vals)

        vals['remaining_qty'] = abs(quantity)
        vals['remaining_value'] = fifo_vals['value']
